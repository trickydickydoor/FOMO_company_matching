...
class CompanyMatcher:
    ...
    def get_cutoff_time(self, hours_ago: int = 36) -> str:
        """
        获取指定小时前的UTC时间戳，用于时间过滤
        
        Args:
            hours_ago: 多少小时前，默认36小时
            
        Returns:
            ISO格式的UTC时间戳字符串
        """
        cutoff_time = datetime.now(timezone.utc) - timedelta(hours=hours_ago)
        return cutoff_time.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'
    ...
    def get_news_batch(self, offset: int, limit: int, cutoff_time: str = None) -> List[Dict[str, Any]]:
        """分批获取新闻数据，支持时间过滤"""
        try:
            query = self.supabase_handler.client.table('news_items')\
                .select('id, content, published_at')
            
            # 如果提供了时间截止点，添加时间过滤条件
            if cutoff_time:
                query = query.gte('published_at', cutoff_time)
            
            response = query.range(offset, offset + limit - 1)\
    ...
    def run_matching(self):
        ...
        # 获取新闻总数（最近36小时）
        cutoff_time = self.get_cutoff_time(36)
        try:
            count_query = self.supabase_handler.client.table('news_items')\
                .select('id', count='exact')
            
            # 添加时间过滤条件
            count_query = count_query.gte('published_at', cutoff_time)
            
            count_response = count_query.execute()
            total_news = count_response.count if count_response.count else 0
            
            # 获取时间范围信息用于日志
            cutoff_datetime = datetime.fromisoformat(cutoff_time.replace('Z', '+00:00'))
            self.logger.info(f"时间过滤条件: 处理 {cutoff_datetime.strftime('%Y-%m-%d %H:%M:%S')} UTC 之后的新闻")
            self.logger.info(f"最近36小时内共有 {total_news} 条新闻需要处理")
        except Exception as e:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for offset in range(0, total_news, batch_size):
                future = executor.submit(self._process_batch_worker, offset, batch_size, cutoff_time)
    ...
    def _process_batch_worker(self, offset: int, batch_size: int, cutoff_time: str = None) -> List[Dict[str, Any]]:
        """批次处理工作线程"""
        try:
            # 获取这一批的新闻数据
            news_batch = self.get_news_batch(offset, batch_size, cutoff_time)
